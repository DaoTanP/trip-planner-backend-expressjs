...
- Route data is derived on demand from ordered stops and their places. Do not persist route segments, route chains, cached polylines, route geometry, route duration, route distance, provider output, or route graph repair state in the planner domain. Persist only lightweight route intent, such as a stop-pair travel mode preference, when user intent must survive reloads or collaboration.
