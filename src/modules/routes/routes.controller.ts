...
import type { ListTripRoutesParams } from '@/modules/routes/routes.schemas.js';
...
export class RoutesController {
  ...
  list = async (req: Request<ListTripRoutesParams>, res: Response) => {
    const routes = await this.service.listRoutes(requireUserId(req), req.params.tripId);
    return sendSuccess(res, { routes: routes.map(serializeRouteSegment) });
  };
}
